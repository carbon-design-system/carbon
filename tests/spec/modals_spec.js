...
describe('Test modal', function () {
  ...
  describe('Showing/hiding', function () {
    ...
    it('Should sanity check the arguments of show()', function () {
      expect(() => {
        modal.show({});
      }).to.throw(Error);
    });
    ...
    it('Should handle ESC key', async function () {
      ...
      expect(eventDataAfterHidden.launchingEventCurrentTarget, 'Launching event for modal-hidden')
        .to.equal(element.ownerDocument.body);
    });

    it('Should not handle non-ESC key for closing', async function () {
      const spyHiddenEventFired = sinon.spy();
      element.classList.add('is-visible');
      element.ownerDocument.body.classList.add('bx--noscroll');
      events.on(element.ownerDocument.body, 'modal-hidden', spyHiddenEventFired);
  ...
  describe('Transition hook', function () {
    ...
    it('Should call the callback of _changeState() after transition ends', function () {
      [...new Array(2)].forEach(() => {
  ...
  describe('Modal in different frame', function () {
    ...
    it('Should not handle ESC key for modals in different frames', async function () {
  ...
  describe('Automatic creation', function () {
    ...
    it('Should do nothing if there is none of a target modal of button upon button click', function () {
      element = document.createElement('a');
      document.body.appendChild(element);
      expect(element.dispatchEvent(new CustomEvent('click', { bubbles: true }))).to.be.true;
    });

    it('Should throw if there are more than one target modals of button', function () {
    ...
    it('Should not focus on modal unless its root element has an area', async function () {
      const id = `__element_${Math.random().toString(36).substr(2)}`;
      element = document.createElement('a');
      element.dataset.modalTarget = `#${id}`;
    ...
    it('Should not cancel event if the button is not <a>', function () {
      const id = `__element_${Math.random().toString(36).substr(2)}`;
