...
5. Once everything is green, review the pull request. Make sure there are no commits labeled `feat` as the patch release should only be for bug fixes, content updates, or refactoring existing code. Double check that the `package.json` versions are updated correctly by verifying the green (updated) version numbers are an incremental increase to the red (deleted) version numbers. If everything is correct, **close** the draft Pull Request. 
