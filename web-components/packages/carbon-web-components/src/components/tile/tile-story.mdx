...
`<bx-radio-tile selected>`) and `false` means not setting the attribute (e.g.
`<bx-radio-tile>` without `selected` attribute).
...
```html
<bx-selectable-tile name="selectable-tile">
  Multi-select Tile
</bx-selectable-tile>
```
...
(e.g. `<bx-selectable-tile>` without `selected` attribute).
...
Note: For `boolean` attributes, `true` means simply setting the attribute (e.g.
`<bx-expandable-tile expanded>`) and `false` means not setting the attribute
(e.g. `<bx-expandable-tile>` without `expanded` attribute).
