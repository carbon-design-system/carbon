...
<cds-tabs trigger-content="Select an item" value="staging">
  ...
  <cds-tab id="tab-router" target="panel-router" value="router">Option 5</cds-tab>
</cds-tabs>
...
For the skeleton variation, utilize `<cds-tabs-skeleton>` and
`<cds-tab-skeleton>`.
...
## `<cds-tabs>` attributes, properties and events

Note: `cds-content-switcher-*` events are not fired from `<cds-tabs>`.

<Props of="cds-tabs" />

## `<cds-tab>` attributes, properties and events

Note: For `boolean` attributes, `true` means simply setting the attribute (e.g.
`<cds-tab disabled>`) and `false` means not setting the attribute (e.g.
`<cds-tab>` without `disabled` attribute).
