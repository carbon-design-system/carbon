...
/ @group @carbon/layout`);
 ...
 LICENSE file in the root directory of this source tree.
`);
        const names = ['box', 'button', 'input'];
        const comment = t.Comment(`/ @type Number
/ @access public
/ @group @carbon/layout`);

        const variables = borderRadius.flatMap((value, index) => {
          const name = `border-radius-${names[index]}`;
          return [
            comment,
            t.Assignment({
...
/ @group @carbon/layout`),
          t.Assignment({
            id: t.Identifier('border-radius'),
            init: t.SassMap({
              properties: borderRadius.map((value, index) => {
                const name = `border-radius-${names[index]}`;
