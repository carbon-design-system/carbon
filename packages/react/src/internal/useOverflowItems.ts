...
const useOverflowItems = <T extends Item>(
  ...
  const overflowItems = useMemo<T[]>(
    () => (Array.isArray(items) ? (items as T[]) : []),
    [items]
  );
  ...
  const visibleItems = useMemo(() => {
    ...
    return overflowItems.slice(0, maxItems).reduce<T[]>((prev, curr) => {
      if (maxReached) {
        return prev;
      }

      const itemWidth = map.get(curr.id) || 0;
  ...
  const hiddenItems = useMemo(
    () => overflowItems.slice(visibleItems.length),
