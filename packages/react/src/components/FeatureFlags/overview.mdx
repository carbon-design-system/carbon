...
| `enable-presence`                                  | Enable components to remain unmounted in closed state and mount in open state.       | `false` | ✅              | ✅        | No - Config only                                                                                                                                               |
...
- **Style only**: Flags like `enable-experimental-tile-contrast` only affect
  Sass styles with no changes to the javascript component API.
- **Config only**: Flags like
  `enable-experimental-focus-wrap-without-sentinels`, `enable-dialog-element`,
  and `enable-treeview-controllable` are configuration options that are enabled
  in just a few specific places in your application.
- **App level**: Flags like `enable-v12-dynamic-floating-styles` are designed to
  be applied at the application level to enable specific behaviors (like
  floating styles without full autoAlign behavior) and don't require widespread
  component changes.
