...
Flags prefixed with `enable-experimental-*` should be considered unstable and
subject to change.

Flags prefixed with `enable-v12-*` are still experimental in nature, but are
unlikely to change. They will be marked as `true` or "on" by default in the next
major version, v12.

For more details on our approach to experimental features, see the
[documentation in the Carbon monorepo](https://github.com/carbon-design-system/carbon/blob/main/docs/experimental-code.md).
...
| `enable-experimental-treeview-controllable`        | Enable the new TreeView controllable API                                 | `false` | ✅              |           |
