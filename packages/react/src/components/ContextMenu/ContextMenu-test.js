  ...
  describe('renders as expected', () => {
    ...
    describe('option', () => {
      ...
      it('renders props.label', () => {
        ...
        expect(wrapper.find(`span.${prefix}--context-menu-option__label`).text()).toBe('Copy');
        expect(wrapper.find(`span.${prefix}--context-menu-option__label`).prop('title')).toBe('Copy');
      });

      it('renders props.renderIcon when provided', () => {
        const wrapper = mount(<ContextMenuOption label="Copy" renderIcon={Copy16} indented />);

        expect(wrapper.find(`div.${prefix}--context-menu-option__icon`).length).toBeGreaterThan(0);
      });

      it('renders props.shortcut when provided', () => {
        const wrapper = mount(<ContextMenuOption label="Copy" shortcut="⌘C" shortcutText="command c" />);

        expect(wrapper.find(`div.${prefix}--context-menu-option__info`).length).toBeGreaterThan(0);
        expect(wrapper.find(`div.${prefix}--context-menu-option__info`).text()).toBe('⌘C');
        expect(wrapper.find(`li.${prefix}--context-menu-option`).prop('aria-label')).toContain('command c');
      });

      it('respects props.disabled', () => {
        const wrapper = mount(<ContextMenuOption label="Copy" disabled />);
        const content = wrapper.find(`button.${prefix}--context-menu-option__content`);

        expect(content.prop('disabled')).toBe(true);
        expect(content.hasClass(`${prefix}--context-menu-option__content--disabled`)).toBe(true);
        expect(wrapper.find(`li.${prefix}--context-menu-option`).prop('aria-disabled')).toBe(true);
      });
      ...
      it('renders props.children as submenu', () => {
        ...
        expect(level1.find(`ul.${prefix}--context-menu`).length).toBeGreaterThan(0);
      });
    });

    describe('radiogroup', () => {
      it('receives the expected classes', () => {
        const wrapper = mount(<ContextMenuRadioGroup label="Share with" items={['None', 'All']} />);
        const container = wrapper.childAt(0);

        expect(container.hasClass(`${prefix}--context-menu-radio-group`)).toBe(true);
      });
    ...
    describe('divider', () => {
      it('receives the expected classes', () => {
        ...
        expect(container.hasClass(`${prefix}--context-menu-divider`)).toBe(true);
      });
