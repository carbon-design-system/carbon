...
export const SidePanel = React.forwardRef<HTMLDivElement, SidePanelProps>(
  (props, ref) => {
    ...
    const [labelTextHeight, setLabelTextHeight] = useState<number>(0);
    ...
    useEffect(() => {
      ...
      if (parentEl) {
        parentEl.style.removeProperty('--cds-side-panel-modified-size');
      }
    }, [size, resizable, sidePanelRef]);
    ...
    }, [sidePanelRef, sidePanelRef?.current?.clientWidth, resizable]);
    ...
    const onResizeEnd = useCallback(() => {
      ...
      if (el) {
        ...
        el.setAttribute('aria-valuenow', String(getPanelWidthPercent()));
      }
      sidePanelWidth.current = sidePanelRef.current?.clientWidth;
    ...
    const renderHeader = () => {
      ...
      if (slug && slug['type']?.displayName === 'AILabel') {
        normalizedDecorator = React.cloneElement(
          slug as React.ReactElement<{ size: string }>,
          {
      ...
      if (aiLabel && aiLabel['type']?.displayName === 'AILabel') {
        normalizedDecorator = React.cloneElement(
          aiLabel as React.ReactElement<{ size: string }>,
          {
      ...
      if (decorator?.['type']?.displayName === 'AILabel') {
        normalizedDecorator = React.cloneElement(
          decorator as React.ReactElement<{ size: string }>,
          {
            ...
            <div className={`${blockClass}__decorator-and-close`}>
              ...
              {!hideCloseButton && (
                <IconButton
                  ...
                  ref={closeRef}
                  align={closeIconTooltipAlignment as PopoverAlignment}>
                  <Close
                    size={16}
                    aria-hidden="true"
                    tabIndex="-1"
        ...
        <Section
          ...
          {!slideIn && resizable && window.innerWidth > 768 && (
            <Resizer
              ...
              aria-label={`side panel is covering ${getPanelWidthPercent()}% of screen`}
              ...
              onResize={(e, delta) => onResize(e, delta)}
