...
component and using it along with any associated table components that you need.

<Preview>
  <Story id="datatable--usage" />
</Preview>

If you don't need the behavior of `DataTable`, or would like to implement your
own behavior, you can also each of the table components exported by Carbon to
construct your own table.

<Preview>
  <Story id="datatable--table-example" />
</Preview>

The `DataTable` component follows the
[`render` prop pattern](https://reactjs.org/docs/render-props.html) to provide
the functionality needed for a data table. As a result, in order to integrate
functionality like sorting, filtering, or row expansion in your table you will
need to use `DataTable` and make sure everything connects for your desired
functionality.
...
You can include `DataTable` and related Table components by writing the
