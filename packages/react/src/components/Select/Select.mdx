...
Use the `inline` variant when the select needs to flow with nearby content.
Inline select fields use a transparent background so they stay consistent with
other inline field inputs.
