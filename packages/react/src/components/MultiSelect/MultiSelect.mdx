...
import { Filterable, FilterableWithCustomSearch } from './MultiSelect.stories';
...
Use `onInputValueChange` to request or calculate custom search results. When the
returned `items` are already filtered, provide a `filterItems` function that
returns them unchanged so Carbon does not apply its default label-based filter a
second time.

<Canvas
  of={FilterableWithCustomSearch}
  additionalActions={[
    {
      title: 'Open in Stackblitz',
      onClick: () => stackblitzPrefillConfig(FilterableWithCustomSearch),
    },
  ]}
/>
