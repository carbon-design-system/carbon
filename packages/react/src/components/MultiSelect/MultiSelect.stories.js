...
export const WithClearIconCallback = (args) => {
  const allItems = [
    { id: 'tag-1', text: 'Apple' },
    { id: 'tag-2', text: 'Banana' },
    { id: 'tag-3', text: 'Cherry' },
    { id: 'tag-4', text: 'Orange' },
  ];
  ...
  const logEvent = (eventName, value) => {
    const message = `${eventName}: ${JSON.stringify(value)}`;
    console.log(message);
    setEvents((previousEvents) => [message, ...previousEvents]);
  };

  // Simulate server-side search: empty results when searching, full list when cleared
  const displayedItems = searchValue ? [] : allItems;

  return (
    <div style={{ padding: '2rem', maxWidth: '700px' }}>
      <div style={{ marginBottom: '1rem' }}>
        <h3 style={{ marginBottom: '0.5rem' }}>
          Server-side Search with Clear Icon (Issue #22316)
        </h3>
        <p style={{ fontSize: '0.875rem', color: '#525252' }}>
          This story demonstrates the fix for the clear icon callback bug. Type
          any search term to see empty results (simulating server-side search),
          then click the clear icon (X). The input is visually cleared and the
          callback now fires correctly, restoring the full list of items.
        </p>
      </div>
      <FilterableMultiSelect
        id="filterable-multiselect-clear-callback"
        titleText="Tags"
        ...
        onInputValueChange={(changes) => {
          const newValue = changes.inputValue || '';
          setSearchValue(newValue);
          logEvent('onInputValueChange', changes);
        }}
        {...args}
      />
      <div style={{ marginTop: '1.5rem' }}>
        <strong>External search state:</strong>
        <pre
          style={{
            padding: '0.5rem',
            background: '#f4f4f4',
      ...
      <div style={{ marginTop: '1rem' }}>
        <strong>Displayed items count:</strong>
        <pre
          style={{
            padding: '0.5rem',
            background: '#f4f4f4',
...
WithClearIconCallback.args = { ...sharedArgs };

WithClearIconCallback.argTypes = {
  ...filterableArgTypes,
};

WithClearIconCallback.parameters = {
  controls: {
    exclude: ['label'],
  },
};
