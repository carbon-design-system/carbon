...
export const ForTestingTitleInheritance = (args) => {
  const itemOnClick = action('onClick (MenuItem)');
  ...
  return (
    <Menu {...args} target={target} x={document?.dir === 'rtl' ? 250 : 0}>
      ...
      />
      <MenuItem
        label="Hover this text & the icon"
        title="This title should NOT be shown when hovering over the icon"
        renderIcon={Copy}
      />
    </Menu>
  );
...
ForTestingTitleInheritance.args = {
  onClose: action('onClose'),
  open: true,
};

ForTestingTitleInheritance.argTypes = {
  target: {
    table: {
      disable: true,
    },
