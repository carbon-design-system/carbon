...
describe('ModalWrapper', () => {
  ...
  it('should set open state to false when close button is clicked', () => {
    ...
    userEvent.click(triggerBtn);
    expect(screen.getByTestId('modal-5')).toHaveClass('is-visible');
  ...
  it('should set open state to false when secondary button is clicked', () => {
    ...
    expect(screen.getByTestId('modal-6')).not.toHaveClass('is-visible');
  });
  ...
  it('should not close if shouldCloseAfterSubmit is false', () => {
    const handleSubmit = jest.fn();
    render(
      <ModalWrapper
        buttonTriggerText="Launch modal"
