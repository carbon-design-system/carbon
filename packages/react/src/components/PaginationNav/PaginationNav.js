...
const getCuts = (totalItems, itemsThatFit, page) => {
  ...
  if (itemsThatFit > 4) {
    if (page <= frontHidden) { // if current page would be in the front overflow
      ...
      frontHidden -= delta;
      backHidden += delta;
    } else if (page >= (totalItems - backHidden - 1)) { // if the current page would be in the back overflow
...
export default class PaginationNav extends Component {
  ...
  renderDirectionButton(direction) {
    ...
    const disabled = !loop && (
      (direction === 'forward' && page === totalItems - 1) || (direction === 'backward' && page === 0)
    );
    const label = direction === 'forward' ? forwardText : backwardText;
    const icon = direction === 'forward' ? CaretRight16 : CaretLeft16;
    const onClick = direction === 'forward' ? this.jumpToNext : this.jumpToPrevious;
        ...
        <li className={`${prefix}--pagination-nav__list-item`}>
          <div className={`${prefix}--pagination-nav__select`}>
            ...
            >
              ...
              <option value="" hidden />
              { [...Array(count)].map((e, i) => (
                <option value={(fromIndex + i).toString()} key={`overflow-${fromIndex + i}`}>
                  { fromIndex + i + 1 }
                </option>
              )) }
            </select>
            <div className={`${prefix}--pagination-nav__select-icon-wrapper`}>
              <OverflowMenuHorizontal16 className={`${prefix}--pagination-nav__select-icon`} />
            </div>
          </div>
        </li>
      );
