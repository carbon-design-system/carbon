...
In cases where an option should be able to be deselected once a selection is made, the `isOptional` prop will append `label`'s content to `items` and render it as the first value in the `Dropdown`.
