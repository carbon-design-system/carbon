...
The `FlexGrid` component supports an optional `withRowGap` prop that adds vertical spacing between rows. When enabled, the row gap automatically matches the current gutter mode:

- **Default (wide)**: 32px row gap
- **Narrow**: 16px row gap
- **Condensed**: 1px row gap

This feature is useful when you want consistent vertical spacing between rows that matches your horizontal gutter spacing.
