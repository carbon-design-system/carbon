...
By default, the CSS Grid does not add vertical spacing (row gap) between grid items. You can enable row gap by using the `withRowGap` prop on the `Grid` component. When enabled, the row gap will automatically match the current gutter size:
...
<Canvas
  ...
  additionalActions={[
    {
      title: 'Open in Stackblitz',
      onClick: () => stackblitzPrefillConfig(GridStories.WithRowGap),
    },
  ]}
/>

_Note: The `withRowGap` prop only works with CSS Grid. It is not available for FlexGrid as the flexbox grid is structured differently._
...
<Canvas
  ...
  additionalActions={[
    {
      title: 'Open in Stackblitz',
      onClick: () => stackblitzPrefillConfig(GridStories.WithGridSettings),
    },
  ]}
/>
