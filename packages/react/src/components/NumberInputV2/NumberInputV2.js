...
import { NumberFormatter, NumberParser } from './NumberInputUtilities';
...
const NumberInputV2 = React.forwardRef(function NumberInputV2(
  ...
  const numberParser = useMemo(() => new NumberParser(locale, formatOptions), [
    locale,
    formatOptions,
  ]);
  ...
  let normalizedDecorator = React.isValidElement(slug ?? decorator)
    ? slug ?? decorator
    : null;
  if (normalizedDecorator?.type?.displayName === 'AILabel') {
...
NumberInputV2.propTypes = {
  ...
  light: deprecate(
    PropTypes.bool,
    'The `light` prop for `NumberInputV2` is no longer needed and has ' +
...
function HelperText({ disabled, description, id }) {
  const prefix = usePrefix();
  const className = classNames(`${prefix}--form__helper-text`, {
