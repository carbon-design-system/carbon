...
export interface FileUploaderItemProps extends HTMLAttributes<HTMLSpanElement> {
  ...
  /**
   * A function used to render custom actions for the file object.
   */
  renderActions?: (props: {
...
function FileUploaderItem({
  ...
  const ariaDescribedBy =
    invalid && errorSubject ? `${filterSpaceName(name)}-id-error` : undefined;
  ...
  const onKeyDown = (evt) => {
    if (matches(evt, [keys.Enter, keys.Space]) && status === 'edit') {
      evt.preventDefault();
  ...
  return (
    <FileUploaderItemBase
      ...
      renderActions={
        renderActions
          ? ({ name, status }) => renderActions({ name, status })
          : () => defaultActions
      }
      renderName={
        renderName ? ({ name }) => renderName({ name }) : () => defaultFileName
      }
      size={size}
      status={status}
      {...other}
