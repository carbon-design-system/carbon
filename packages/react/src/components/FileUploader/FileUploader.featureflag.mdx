...
the `getCurrentFiles` and `setCurrentFiles` methods are available on the `FileUploader` component, which can be used to get and update the file state and to disable specific files based on disabled prop.
