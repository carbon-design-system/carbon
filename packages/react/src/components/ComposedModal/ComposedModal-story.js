...
export const UsingHeaderFooterProps = () => {
  ...
  return (
    <ComposedModal {...rest} danger={true} size={size || undefined}>
      ...
      <ModalBody {...bodyProps} aria-label={hasScrollingContent ? 'Modal content' : undefined}>
  ...
  return (
    <ComposedModal {...rest} size={size || undefined}>
      ...
      <ModalFooter>
        ...
        <Button kind={props.composedModal().danger ? 'danger' : 'primary'}>Save</Button>
      </ModalFooter>
    </ComposedModal>
  );
};
...
export const ExampleUsageWithTriggerButton = () => {
  class ComposedModalExample extends React.Component {
    ...
    render() {
      ...
      return (
        <>
          <Button onClick={() => this.toggleModal(true)}>Launch composed modal</Button>
          <ComposedModal
            {...rest}
            open={open}
            size={size || undefined}
