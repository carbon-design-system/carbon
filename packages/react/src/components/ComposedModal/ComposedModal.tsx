...
import {
  ...
  useRegisterWithComposedModalPresenceContext,
...
const ComposedModal = React.forwardRef<HTMLDivElement, ComposedModalProps>(
  function ComposedModal({ open, ...props }, ref) {
    ...
    const isRegisteredWithPresenceContext =
      useRegisterWithComposedModalPresenceContext();

    if (hasPresenceContext && !isRegisteredWithPresenceContext) {
      return (
        <ComposedModalPresence open={open ?? false}>
    ...
    if (enablePresence && !isRegisteredWithPresenceContext) {
      return (
        <ComposedModalPresence open={open ?? false} autoEnablePresence={false}>
          <ComposedModalDialog open ref={ref} {...props} />
        </ComposedModalPresence>
      );
    }
