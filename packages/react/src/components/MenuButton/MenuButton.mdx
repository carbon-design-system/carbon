...
## menuTarget Prop usage with React refs
When using `menuTarget` with a React ref, you need to use state to capture the ref value.
This is because React doesn't detect changes to `ref.current` properties for prop updates:
```jsx
// ✅ Correct usage
...
const containerRef = useRef(null);
<div ref={containerRef}>
  <MenuButton label="Actions" menuTarget={containerRef.current}>
    <MenuItem label="First action" />
  </MenuButton>
</div>
```
