...
[Source
code](https://github.com/carbon-design-system/carbon/tree/main/packages/react/src/components/Tooltip/next)
&nbsp;|&nbsp;
...
<Canvas>
  <Story id="components-tooltip--default" parameters={{ layout: "centered" }} />
</Canvas>

### Customizing the content of a tooltip

The `Tooltip` component supports tooltip customization through the `label`
and `description` props. Under the hood, `label` will map to `aria-labelledby`
and `description` will map to `aria-describedby`.
...
information shown is the only content that describes your component, you should use a
`label`. If the information is secondary or adds additional information,
