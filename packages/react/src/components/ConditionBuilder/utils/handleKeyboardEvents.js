...
export const handleKeyDown = (
  ...
  if (
    ...
    handleKeyPressForMainContent(evt, conditionBuilderRef, variant, blockClass);
  }
};
...
export const handleKeyDownForPopover = (
  ...
  if (excludeKeyPress(evt, blockClass)) {
    return;
  }
...
const excludeKeyPress = (evt, blockClass) => {
  return (
...
const handleKeyPressForMainContent = (
  evt,
  conditionBuilderRef,
  variant,
  blockClass
  ...
  switch (evt.key) {
    case 'ArrowRight':
      ...
      if (variant == HIERARCHICAL_VARIANT) {
        ...
        if (allCellsInRow.length === 1) {
          ...
          handleRowNavigationHierarchical(
            evt,
            conditionBuilderRef,
            variant,
            blockClass
        ...
        handleCellNavigation(evt, conditionBuilderRef, blockClass);
      }
      break;
    case 'ArrowLeft':
      evt.preventDefault();
      if (variant == HIERARCHICAL_VARIANT) {
        if (evt.target.getAttribute('role') !== 'row') {
          ...
          if (currentItemIndex > 0) {
            ...
            manageTabIndexAndFocus(wrapper, conditionBuilderRef, blockClass);
          }
        }
      } else {
        handleCellNavigation(evt, conditionBuilderRef, blockClass);
    ...
    case 'ArrowDown':
      ...
      if (variant == HIERARCHICAL_VARIANT) {
        ...
        handleRowNavigation(evt, conditionBuilderRef, variant, blockClass);
      }
...
const handleRowNavigation = (evt, conditionBuilderRef, variant, blockClass) => {
  const rows = getRows(conditionBuilderRef);
...
const handleRowNavigationHierarchical = (
  evt,
  conditionBuilderRef,
  variant,
  blockClass
...
const navigateToNextRowCell = (
  ...
  if (nextRow?.querySelector(`[data-name="${itemName}"]`)) {
    ...
    manageTabIndexAndFocus(nextRow, conditionBuilderRef, blockClass);
  }
};
const handleCellNavigation = (evt, conditionBuilderRef, blockClass) => {
