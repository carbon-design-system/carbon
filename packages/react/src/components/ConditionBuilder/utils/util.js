...
export const focusThisField = (evt, conditionBuilderRef, blockClass) => {
  if (evt) {
    ...
    setTimeout(() => {
      ...
      manageTabIndexAndFocus(selector, conditionBuilderRef, blockClass);
...
export const focusThisItem = (
  currentElement,
  conditionBuilderRef,
  blockClass
) => {
  setTimeout(() => {
    manageTabIndexAndFocus(currentElement, conditionBuilderRef, blockClass);
  }, 0);
};
export const traverseClockwise = (
  ...
  if (eachElem == document.activeElement) {
    if (index !== allElements.length - 1) {
      focusThisItem(allElements[index + 1], conditionBuilderRef, blockClass);
    } else {
      focusThisItem(
    ...
    focusThisItem(allElements[0], conditionBuilderRef, blockClass);
  }
};
...
export const manageTabIndexAndFocus = (
  currentElement,
  conditionBuilderRef,
  blockClass
) => {
  ...
  if (contentContainer) {
    Array.from(contentContainer.querySelectorAll('[tabindex="0"]')).forEach(
