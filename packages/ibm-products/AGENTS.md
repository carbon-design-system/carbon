...
`@carbon/ibm-products` is intended to be a single point of entry for most
everything a consumer needs to build a React app with the Carbon Design System.
Styles are re-exported from `packages/ibm-products-styles`.
...
For `packages/ibm-products/src/components/`:
