 ...
 *
 * Replicates the output of `tasks/builders/dtcg-themes.js` as a Style
 * Dictionary format function.
 *
 ...
 *   platforms: {
 *     'scss/themes/white': {
 *       prefix: 'white',        ← used as the Sass variable name
 *       files: [{
 *         destination: 'scss/generated/_themes.scss',
 *         format: 'carbon/scss-themes',
 *         options: { themeName: 'white' }
 *       }]
 *     }
 *   }
 *
 * The `themeName` option defaults to the platform prefix when omitted.
 *
 * Output (abbreviated):
 *
 ...
 *     ...
 *   ) !default;
 *   $white: utilities.merge($white, layout.$spacing, layout.$fluid-spacing, type.$tokens);
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * NOTE ON MULTI-THEME FILES
 * ─────────────────────────────────────────────────────────────────────────────
 * The current build writes ALL four themes into a SINGLE _themes.scss file via
 * dtcg-themes.js.  SD cannot do that in a single format call unless all four
 * platform builds write to the same destination with `append` mode.  The
 * recommended approach:  run all four platforms, collect their individual
...
// Banner matches shared.js exactly: year 2023, trailing '//' line.
