...
describe('Homepage -', ()=> {
  it('Check Getting started hyperlink to be clickable', () => {
    const {queryByLabelText, getByLabelText, getByTestId, getByRole} = render(<HomePage />);
  ...
  it('Check tab switching', async() => {
    ...
    await screen.getByText(/Rapidly build beautiful and accessible experiences/i);
    //swith another tab;
    const developTab = screen.getByText(/Develop/i);
    fireEvent.click(developTab);
    await screen.getByText(/Carbon provides styles and components in Vanilla, React, Angular, and Vue for anyone building on the web/i)
  })
})
