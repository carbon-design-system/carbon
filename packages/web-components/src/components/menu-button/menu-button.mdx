...
actions must be MenuItems passed as children. The trigger buttons's label is
passed as `label`.
...
The `menu-alignment` prop enables you to define the placement of the Menu in
relation to the MenuButton. For instance, setting menu-alignment="top" on the
...
understand the `menu-alignment` prop.
