 ...
 * NotificationPanel.
 *
 * @element cds-notification-panel
 * @csspart dialog The dialog.
 * @slot today - Today Section.
 * @slot previous - Previous Section.
 * @slot footer - Footer for the Panel.
 * @fires cds-notification-panel-beingclosed - The custom event before notification-panel is closed.
 * @fires cds-notification-dismiss-all - The custom event fired after notification-panel is closed upon a user gesture.
...
class CDSNotificationPanel extends HostListenerMixin(LitElement) {
   ...
   */
  @property({ reflect: true, type: String, attribute: 'today-text' })
  todayText;
  /**
   * Sets the Previous section title for the Notification panel
   */
  @property({ reflect: true, type: String, attribute: 'previous-text' })
  previousText;
  /**
   ...
   */
  @property({ reflect: true, type: String, attribute: 'dismiss-all-label' })
  dismissAllLabel;
  /**
   * Sets the empty state label text when there are no notifications
   */
  @property({ reflect: true, type: String, attribute: 'empty-state-label' })
  emptyStateLabel;
  /**
   * Sets the label text for the "Do Not Disturb" toggle in the Notification panel
   */
  @property({ reflect: true, type: String, attribute: 'donot-disturb-label' })
  doNotDisturbLabel;
  /**
   ...
   */
  @property({ reflect: true, type: String, attribute: 'date-time-locale' })
  dateTimeLocale?: string;

  @provide({ context: dateTimeLocaleContext })
  // @ts-expect-error: @provide writes this field via the decorator; TS doesn't see it as read
      ...
      <div role="dialog" tabindex="0" class=${classes}>
        <div class="${blockClass}__header-container">
          ...
          <cds-toggle
            ...
            @cds-toggle-changed=${handleToggle}></cds-toggle>
        </div>
