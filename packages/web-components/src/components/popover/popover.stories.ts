...
import { property, state } from 'lit/decorators.js'; // remove before merging
  ...
  render() {
    return html`
      ...
      <div class="button-container">
        <cds-button
          class="next-btn ${this.showNext ? '' : 'hidden'}"
          size="sm"
          @click=${this.handleNext}>
          Next
        </cds-button>
        <cds-button
          class="done-btn ${this.showDone ? '' : 'hidden'}"
          size="sm"
          @click=${this.handleDone}>
          Done
        </cds-button>
        <input />
        <cds-button class="done-btn" size="sm" @click=${this.handleDone}>
          Done
        </cds-button>
      </div>
    `;
...
customElements.define('nested-button-container', NestedButtonContainer);

export class TestPopoverExample extends LitElement {
  @state()
  open: boolean = true;
  ...
  private handleDone() {
    // eslint-disable-next-line no-console
    console.log(
      'Done clicked - popover should stay open but might close due to shadow DOM boundary'
    );
  }
    ...
    return html`
      ...
      <cds-popover ?open=${this.open}>
        ...
        <cds-popover-content slot="content">
          <cds-layer>
            <p class="popover-title">Shadow DOM Focus Test</p>
            <p class="popover-details">
              Click "Next" to hide it and show "Done" button. This simulates the
              coachmark scenario with nested shadow DOM.
            </p>
            <nested-button-container
              @done-clicked=${this.handleDone}></nested-button-container>
          </cds-layer>
        </cds-popover-content>
      </cds-popover>
    `;
...
customElements.define('test-popover-example', TestPopoverExample);

export class MyApp2 extends LitElement {
  render() {
    return html` <div style="padding-top:200px; position: relative;">
      <test-popover-example></test-popover-example>
    </div>`;
  }
}

customElements.define('my-app-2', MyApp2);
