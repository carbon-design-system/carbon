...
class CDSMenu extends HostListenerMixin(LitElement) {
  ...
  getMenuItems(assignedNodes) {
    Array.from(assignedNodes as Element[])?.forEach((node) => {
      let menuItems;
      if (node?.tagName === 'CDS-MENU-ITEM') {
        menuItems = node;
        // node.shadowRoot
        // ?.querySelector(`.${prefix}--menu-item`);
        this.menuItems = [...this.menuItems, menuItems];
      } else if (node?.tagName === 'CDS-MENU-ITEM-SELECTABLE') {
        menuItems = node.shadowRoot?.querySelector('cds-menu-item');
        // ?.shadowRoot
        //   ?.querySelector(`.${prefix}--menu-item`);
        this.menuItems = [...this.menuItems, menuItems];
      } else if (node?.tagName === 'CDS-MENU-ITEM-RADIO-GROUP') {
        this.getMenuItems(node?.shadowRoot?.querySelectorAll('cds-menu-item'));
      } else if (node?.tagName === 'CDS-MENU-ITEM-GROUP') {
        const menuItemSelectables = node.shadowRoot?.querySelectorAll(
          'cds-menu-item-selectable,cds-menu-item'
        );
        Array.from(menuItemSelectables || []).forEach((menuItemSelectable) => {
          if (menuItemSelectable?.tagName === 'CDS-MENU-ITEM') {
            this.getMenuItems([menuItemSelectable]);
          } else {
            this.getMenuItems([
              menuItemSelectable?.shadowRoot?.querySelector('cds-menu-item'),
            ]);
          }
        });
      }
  ...
  getActiveItem(node) {
    if (node?.tagName === 'CDS-MENU-ITEM') {
      ...
      return node.shadowRoot?.querySelector('cds-menu-item');
    } else if (node?.tagName === 'CDS-MENU-ITEM-RADIO-GROUP') {
      this.getMenuItems(node?.shadowRoot?.querySelectorAll('cds-menu-item'));
    } else if (node?.tagName === 'CDS-MENU-ITEM-GROUP') {
      const menuItemSelectables = node.shadowRoot?.querySelectorAll(
  ...
  _focusItem = (e: KeyboardEvent | undefined) => {
    ...
    const activeItem = getActiveItem(document.activeElement);
    currentItem = this.menuItems.findIndex(
    ...
    if (indexToFocus < 0) {
      indexToFocus = this.menuItems.length - 1;
    }
    if (indexToFocus >= this.menuItems.length) {
      indexToFocus = 0;
    }

    if (indexToFocus !== currentItem) {
      const nodeToFocus = this.menuItems[indexToFocus];
      (
