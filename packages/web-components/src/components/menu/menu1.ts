  ...
  applyIconClass(assignedNodes) {
    Array.from(assignedNodes as Element[])?.forEach((node) => {
      if (node?.tagName === 'CDS-MENU-ITEM') {
        ...
        this.applyIconClass(
          node?.shadowRoot?.querySelectorAll('cds-menu-item')
        );
        ...
        Array.from(menuItemSelectables || []).forEach((menuItemSelectable) => {
          if (menuItemSelectable?.tagName === 'CDS-MENU-ITEM') {
            this.applyIconClass([menuItemSelectable]);
          } else {
            this.applyIconClass([
  ...
  connectedCallback() {
    ...
    this.addEventListener('icon-detect', () => {
      const assignedNodes = this.shadowRoot
        ...
        ?.assignedElements();
      // this.shadowRoot
      //  this.applyIconClass(assignedNodes);
      this.shadowRoot
