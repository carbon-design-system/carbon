...
- Use `cds-disimissible-tag` when you want to allow users to easily clear or
  dismiss specific filters applied to items.
    ...
    ${tags.map(
      (tag) => html`
        <cds-dismissible-tag
          ...
          ${iconLoader(Asleep16, { slot: 'icon' })} ${tag.text}
        </cds-dismissible-tag>
      `
    )}
  `;
...
`<cds-tag>` with AI label is now stable for Read Only tags. This addition
