...
/**
 * Keyboard focus regression tests for cds-preview-date-picker (Issue #1174).
 *
...
describe('cds-preview-date-picker keyboard navigation – Tab from calendar (Issue #1174)', function () {
  // Allow extra time for ES-module loading when multiple test files run concurrently.
  this.timeout(10000);

  /**
   * Build a page scaffold:
   *   [button#sentinel-before] [cds-preview-date-picker] [button#sentinel-after]
   *
  ...
  async function buildFixture() {
    const container = await fixture(html`
      <div>
        ...
        <cds-preview-date-picker id="test-picker">
   ...
   * @param {HTMLElement} picker - The cds-preview-date-picker element under test.
   */
  ...
  it('focusing the exit sentinel closes the calendar and restores tabindex="-1"', async () => {
    ...
    expect(
      picker.shadowRoot?.querySelector('cds-preview-date-picker-calendar')
    ).to.equal(null);
  ...
  it('Shift+Tab from the calendar moves focus back to the date text input', async () => {
    ...
    const datePickerInput = picker.querySelector(
      'cds-preview-date-picker-input'
    );
