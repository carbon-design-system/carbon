...
export function format(
  ...
  if (Math.abs(days) < 365) {
    // A month is approximated as 28 days above, so the day before the year
    // branch takes over (364 days) yields 13 months. Clamp to a valid month
    // magnitude so the output never exceeds 12 months.
    const clampedMonths = Math.max(-12, Math.min(12, months));
    return rtf.format(clampedMonths * -1, 'months');
  }
