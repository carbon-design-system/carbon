...
const { white } = require('@carbon/elements');
...
const experimentalVariables = Object.keys(white).map(key => {
  return paramCase(key).replace(/(\d+)/g, '-$1');
});

const experimental = new Set([...classic, ...experimentalVariables]);
...
`);
  ...
  describe('experimental', () => {
    test.each([...experimental])('$%s should be exported', async name => {
      const { calls } = await renderSass(`
